...
public class SpawnElytraCraftattack implements ModInitializer {
    ...
    private void handlePlayerFlight(ServerPlayer player) {
        ...
        boolean isRiding = player.getVehicle() != null; // <-- Spieler sitzt auf etwas

        if (flightData != null && flightData.inFlight) {
            player.fallDistance = 0;
        }

        // Reset bei Landung, Wasser oder Sitzen
        if ((onGround || inWater || isRiding) && flightData != null) {
            if (currentArea != null) flightData.boostsRemaining = currentArea.maxBoosts;
            else flightData.boostsRemaining = 0;
        ...
        // Flug aktivieren nach 500ms nur wenn Spieler wirklich in Luft und nicht sitzt
        if ((currentArea != null || (flightData != null && flightData.inFlight)) && !isFlying && !inWater && !isRiding) {
            long inAirStart = PLAYER_INAIR.getOrDefault(uuid, System.currentTimeMillis());
            if (!onGround) {
                if (!PLAYER_INAIR.containsKey(uuid)) PLAYER_INAIR.put(uuid, System.currentTimeMillis());
                long elapsed = System.currentTimeMillis() - inAirStart;
                if (elapsed >= 500) {
                    player.startFallFlying(); // Custom Elytra Animation
                    if (flightData == null && currentArea != null) {
                        PLAYER_FLIGHTS.put(uuid, new PlayerFlightData(currentArea.name, currentArea.maxBoosts));
                        flightData = PLAYER_FLIGHTS.get(uuid);
                    }
                    if (flightData != null) flightData.inFlight = true;
                }
            } else {
                PLAYER_INAIR.remove(uuid);
            }
    ...
    private void handlePlayerBoost(ServerPlayer player, PlayerFlightData flightData) {
        ...
        if (isSneaking && !flightData.wasSneaking) {
            ...
            // Vanilla Stufe-1 Rakete: horizontal ~1.5, vertikal ~1.0
            ...
            // Direkt auf DeltaMovement anwenden
